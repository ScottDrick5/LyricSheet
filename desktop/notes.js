...
async function notesIn(folderId) {
  ...
  const out = await osa([
    ...
    // (variable names here must not be Notes words: \"locked\" is one, and setting it failed)
    "      set noteLocked to false",
    "      set noteBody to \"\"",
    "      set noteOk to \"ok\"",
    "      try",
    "        set noteLocked to password protected of n",
    "      end try",
    "      try",
    "        if not noteLocked then set noteBody to body of n",
    "      on error",
    "        set noteOk to \"unreadable\"",
    "      end try",
    "      try",
    "        set out to out & (id of n) & US & (name of n) & US & ((creation date of n) - nowD) & US & ((modification date of n) - nowD) & US & noteLocked & US & noteOk & US & noteBody & RS",
    "      end try",
    "    end repeat",
    "  end tell",
    "  return out",
  ...
  return out.split(RS).filter((r) => r.trim()).map((r) => {
    ...
    return {
      ...
      locked: f[4] === "true",
      unreadable: f[5] === "unreadable",
      html: f.slice(6).join(US),
    };
  });
}
